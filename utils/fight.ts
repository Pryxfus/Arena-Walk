...
export const fight = (warrior1: WarriorRecord, warrior2: WarriorRecord): {
    ...
    do {
        ...
        if (defender.dp + defender.warrior.agility > attackStrength) {
            ...
            if (defender.dp < 0) {
                log.push(`${attacker.warrior.name} przełamał obronę ${defender.warrior.name} zadając mu ${-defender.dp}.`);
    ...
    const winner = defender.warrior; // atakujący czyli obrońca bo mamy powyżej tablicę obracanych BIG BRAIN TIME :D
