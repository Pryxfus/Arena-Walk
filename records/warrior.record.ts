...
export class WarriorRecord {
    ...
    constructor(obj: Omit<WarriorRecord, 'insert' | 'update'>) {
        const {id, name, defence, power, stamina, agility, wins} = obj;
        ...
        for(const stat of stats) {
            if (stat < 1) {
                throw new ValidationError('Każda ze statystyk musi wynosić 1. Ta zasada została złamana.')
            }
        }
    ...
    static async getOne(id: string): Promise<WarriorRecord |null> {
       const [results] = await pool.execute("SELECT * FROM `warriors` WHERE `id` = :id", {
           id: id, // nie używamy this.id bo to metoda statyczna
